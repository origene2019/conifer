...
def loadProbeList(CF_probe_filename):
    with open(CF_probe_filename, newline='') as csvfile:
        reader = csv.DictReader(csvfile)
        probes = []
        probeID = 1
        for row in reader:
            probes.append({'probeID': probeID, 'chr':int(row['chr']),'start':int(row['start']),'stop':int(row['stop']), 'name':row['name']})
            probeID +=1
    if len(probes) == 0:
        Exception("No probes in probe file")
    return probes
...
class rpkm_reader:
	...
	def getExonValuesByExons(self, chromosome, start_exon, stop_exon, sampleList=None,genotype=False):
		...
		probe_tbl = self.h5file.root._f_get_child("chr" + str(chromosome))
