...
def CF_analyze(args):
	...
	for i,s in enumerate(samples.keys()):
		#filecp = codecs.open(samples[s], encoding = 'cp1252')
	...
	for chr in chrs_to_process:
		...
		#	print "[WARNING] This chromosome has fewer informative probes than there are samples in the analysis! There are probably no mappings on this chromosome. Please remove these probes from the probes.txt file"
		#	sys.exit(0)
...
def CF_bam2RPKM(args):
	...
	#if max(exon_bp) != 0 and total_reads != 0 and max(readcount) != 0:
